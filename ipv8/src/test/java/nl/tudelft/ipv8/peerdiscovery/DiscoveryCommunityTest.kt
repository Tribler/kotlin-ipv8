...
class DiscoveryCommunityTest : BaseCommunityTest() {
    ...
    fun createSimilarityRequest() {
        ...
        community.onSimilarityRequestPacket(Packet(IPv4Address("1.2.3.4", 1234), payload))
    ...
    fun createSimilarityResponse() {
        ...
        community.onSimilarityResponsePacket(Packet(IPv4Address("1.2.3.4", 1234), payload))
    ...
    fun createPing() {
        ...
        community.onPingPacket(Packet(IPv4Address("1.2.3.4", 1234), payload))
    ...
    fun createPong() {
        ...
        community.onPongPacket(Packet(IPv4Address("1.2.3.4", 1234), payload))
    ...
    fun onSimilarityResponse_maxPeers() {
        ...
        val endpoint = getEndpoint()
        val network = Network()
        network.addVerifiedPeer(Peer(JavaCryptoProvider.generateKey(), IPv4Address("12.2.3.4", 5234)))
        val community = DiscoveryCommunity()
        community.myPeer = myPeer
        community.endpoint = endpoint
        community.network = network
        ...
        val peer1 = Peer(JavaCryptoProvider.generateKey(), IPv4Address("13.2.3.4", 5234))
        ...
        community.onSimilarityResponsePacket(Packet(IPv4Address("13.2.3.4", 1234), payload1))
        assertEquals(1, community.getPeers().size)
        assertEquals(2, network.verifiedPeers.size)

        val peer2 = Peer(JavaCryptoProvider.generateKey(), IPv4Address("14.2.3.4", 5234))
        ...
        community.onSimilarityResponsePacket(Packet(IPv4Address("14.2.3.4", 5234), payload2))
